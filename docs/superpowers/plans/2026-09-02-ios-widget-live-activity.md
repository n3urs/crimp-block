...
- **Deployment target:** `16.4`, matching the main app's current `IPHONEOS_DEPLOYMENT_TARGET` (already ≥ 16.1, the floor for Live Activities) — do not accept the plugin's own default (18.0), set it explicitly in `expo-target.config.js`.
